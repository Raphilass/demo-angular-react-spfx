...
export class AddHolidayPanelComponent implements AfterViewInit, OnInit {
  ...
  @Input("CurrentUser") _currentUserEmail:string;
  ...
  _handleAddAbsence(){
    let item:IAbsence = {
      Title:this.reason,
      Category:this.type,
      EndDate:this.endDate.date.toISOString(),
      EventDate:this.startDate.date.toISOString(),
      fAllDayEvent: true
    }
    this.absenceProvider.AddAbsence(this._listId,item, this._currentUserEmail)
