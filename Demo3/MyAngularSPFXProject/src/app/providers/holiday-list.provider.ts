...
import { sp, Field, ChoiceFieldFormatType, ItemAddResult, WebEnsureUserResult } from '@pnp/sp';
...
export class HolidayListProviderService {
    ...
    public AddAbsence(listId:string, item:any, userId:string):Observable<ItemAddResult>{
        return from(
            sp.web.ensureUser(userId)            
        ).pipe(
        switchMap((user:WebEnsureUserResult)=>{
            console.log()
            item.EmployeeId = user.data.Id;

            return from(sp.web.lists
            .getById(listId)
            .items
            .add(item))
            })
